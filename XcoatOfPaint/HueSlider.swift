...
public class HueSlider: NSSlider {
    ...
    override public func draw(_ dirtyRect: NSRect) {
        ...
        let bar = NSBezierPath(roundedRect: bezelFrame,
                               xRadius: bezelFrame.height * 0.5,
                               yRadius: bezelFrame.height * 0.5)
        ...
        let shadowPath = NSBezierPath(roundedRect:
                                        NSRect(x: (knobX - bounds.height / 4), y: 0, width: 10, height: bounds.height)
                                        .insetBy(dx: 0.5, dy: 3.5), xRadius: 5, yRadius: 5)
        NSColor(white: 0.3, alpha: 0.3).setFill()
        shadowPath.fill()

        let knobPath = NSBezierPath(roundedRect:
                                        NSRect(x: (knobX - bounds.height / 4), y: 0, width: 10, height: bounds.height)
                                        .insetBy(dx: 1, dy: 4), xRadius: 5, yRadius: 5)
